...
// --- the tip jar, counted by whoever is standing at it ----------------------
//
// A barista keeps the cash their own guests hand them. The jar on the counter
// is a different pot with a rule of its own, and only a manager could enter
// it — the one person not standing next to it at close.

const JAR_POLICY = NEW_SHAPE.concat([
  { type: 'pool', source: 'jar', split: 'hours', among: ['barista'], payout: 'weekly_cash' },
]);

test('a barista is asked for the jar where the policy pays a jar out', async () => {
  const pin = baristaPin();
  const sh = serviceOn('2026-10-10', 'cafe', JAR_POLICY, 'jar to baristas', 'Cleo Barista', 'barista');
  const labels = await labelsFor(pin, sh, 'barista');
  assert.ok(labels.some((l) => /Cash from the tip jar/.test(l)), 'the jar is asked for');
  assert.ok(labels.some((l) => /already took home/.test(l)),
    'and their OWN cash is a separate question — two pots, two boxes');
});

test('and not where there is no jar to pay out', async () => {
  const pin = baristaPin();
  const sh = serviceOn('2026-10-11', 'dinner', NEW_SHAPE, 'no pools', 'Cleo Barista', 'barista');
  const labels = await labelsFor(pin, sh, 'barista');
  assert.ok(!labels.some((l) => /tip jar/.test(l)), 'no box for a pot that does not exist');
});

test('and not to somebody the jar rule does not name', async () => {
  // Counting a jar you are not in is somebody else's job.
  const sh = serviceOn('2026-10-12', 'cafe', JAR_POLICY, 'jar to baristas', 'Tess Blake', 'bartender');
  const labels = await labelsFor('1470', sh, 'bartender');
  assert.ok(!labels.some((l) => /tip jar/.test(l)), 'the bartender is not asked');
});

test('the jar a barista counts is pooled; the cash they were tipped is not', async () => {
  const pin = baristaPin();
  const sh = serviceOn('2026-10-13', 'cafe', JAR_POLICY, 'jar to baristas', 'Cleo Barista', 'barista');
  const start = await form('/tips/start', { pin });
  const cookie = (start.headers.get('set-cookie') || '').split(';')[0];
  const open = await fetch(`${BASE}/portal/tips?shift=${sh}&position=barista`, { headers: { cookie } });
  const token = (await open.text()).match(/name="token" value="([^"]+)"/)[1];
  const res = await fetch(`${BASE}/portal/tips/submit`, {
    method: 'POST', redirect: 'manual',
    headers: { cookie, 'content-type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({ token, shift_id: String(sh), position: 'barista',
      food: '0', coffee: '500', alcohol: '0', card_tips: '80',
      cash_tips: '25', jar_cash: '60' }).toString(),
  });
  assert.ok([200, 302].includes(res.status), `accepted (got ${res.status})`);

  const r = new Database(DB, { readonly: true });
  const row = r.prepare(`SELECT ss.* FROM server_sales ss JOIN employees e ON e.id = ss.employee_id
    WHERE ss.shift_id = ? AND e.name = 'Cleo Barista'`).get(sh);
  r.close();
  assert.strictEqual(row.cash_tips_cents, 2500, 'their own cash is their own');
  assert.strictEqual(row.jar_cash_cents, 6000, 'the jar is the jar — a different column');

  // And the running app treats them as the two different things they are.
  // Asked of the SERVER rather than by requiring src/db here — this file does
  // not set DB_PATH, so an in-process require would read the developer's own
  // database instead of the fixture.
  const sheet = await (await fetch(`${BASE}/shifts/${sh}`)).text();
  assert.match(sheet, /Cash pool[\s\S]{0,200}\$60\.00/, 'the jar, and only the jar, is the cash pool');
  assert.match(sheet, /staff counted <\/?b?i?>?\$60\.00|staff counted[^<]*\$60\.00/,
    'and the sheet says a member of staff counted it, not the manager');
  assert.ok(!/Cash pool[\s\S]{0,200}\$85\.00/.test(sheet),
    'their own $25 was never swept in on top of it');
});
